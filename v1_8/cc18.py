...
def log_error(exception, output_dir):
    """Log detailed error info to a text file in output_dir."""
    log_file = os.path.join(output_dir, 'Cash_Crawler_Error_log.txt')
    with open(log_file, 'a') as f:
        f.write("Please submit this log file to North Loop Consulting at northloopconsulting.com for support.\nUnder Contact Us on the home page you can copy/past this information or attach the file.\n")
        f.write("=" * 80 + "\n")
        f.write("Error occurred at: " + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "\n")
...
class ActivityIndicator(ttk.Label):
    def __init__(self, parent):
        super().__init__(parent)
        self._angle = 0
        self.running = False
        
        # Create smaller spinner image
        size = 25  # Reduced from 80
        self.image = Image.new('RGBA', (size, size), (0,0,0,0))
        draw = ImageDraw.Draw(self.image)
        
        bbox = [1, 1, size-2, size-2]
        draw.arc(bbox, 0, 360, fill='blue', width=2)
        draw.arc(bbox, 315, 360, fill=(0,0,0,0), width=2)
    ...
    def _spin(self):
        if self.running:
            self._angle = (self._angle + 30) % 360  # Increased from 10
            rotated = self.image.rotate(self._angle)
            self.photo = ImageTk.PhotoImage(rotated)
            self.configure(image=self.photo)
            self.after(20, self._spin)  # Decreased from 50
    def stop(self):
        self.running = False

class ProgressHandler:
    def __init__(self, spinner, status_label, root):
...
template_path = get_resource_path('report_template_1_5.html')
    ...
    if associate_files:
        for file in associate_files:
            ...
            try:
                ...
                messagebox.showerror("Error",f"Error reading {tokenname} data. Index Error related to Excel sheet")
                continue
...
def collect_account_data(datasets):
    ...
    #process "Payment Source History" dataset 
    if 'Payment Source History' in datasets:
        payment_source_df = datasets.get('Payment Source History', pd.DataFrame())
        if not payment_source_df.empty:
            # Clean column names
            payment_source_df.columns = payment_source_df.columns.astype(str).str.strip()

            # Separate card-based entries
            card_cols = {'Card', 'Card Brand'}
            if card_cols.issubset(payment_source_df.columns):
                card_df = payment_source_df[list(card_cols)].dropna().drop_duplicates()
                account_ids["Payment Source - Cards"] = card_df.to_dict('records')
        else:
            # print("'Payment Source History' dataset is missing.")
            pass
    else:
        print("'Payment Source History' dataset is missing.")

    #### Need to add support for routing number and account number #####
    
    #process Devices  First Seen	Name	UUID
    if 'Devices' in datasets:
        devices_df = datasets.get('Devices', pd.DataFrame())
        if not devices_df.empty:
            # Clean column names
            devices_df.columns = devices_df.columns.astype(str).str.strip()

            # Check if all required columns are present
            required_columns = ['Name', 'UUID', 'Platform']
            if all(col in devices_df.columns for col in required_columns):
                account_ids["Devices"] = devices_df[['Name', 'UUID', 'Platform']].dropna().to_dict('records')
            else:
                    missing_columns = [col for col in required_columns if col not in devices_df.columns]
                    print(f"Missing columns in 'Devices': {', '.join(missing_columns)}")
        else:
            print("'Devices' dataset is missing.")
    ...
    try:
        ...
        if "Bad password" in str(e):
            messagebox.showerror("Password Error", 
                "Invalid password provided for ZIP file. Please check and try again.")
        else:
...
def join_dataframes(dataframes):
    ...
    if df_list:
        ...
        joined_df = pd.DataFrame()  # Return empty DataFrame if no datasets found
...
def bitcoin_transactions(dataframes):
    ...
    if 'Attempted Bitcoin Transactions' not in dataframes:
        print("'Attempted Bitcoin Transactions' DataFrame not found in the provided dataframes. Skipping Bitcoin stats.")
        return pd.DataFrame()
    ...
    if wallet_column is None:
        print("No wallet address column found in Bitcoin transactions.")
        return pd.DataFrame()

    try:
        bitcoin_df = df.copy()
        bitcoin_df['Amount'] = bitcoin_df['Amount'].str.replace('BTC ', '').str.replace(',', '', regex=False).astype(float)
        bitcoin_df[wallet_column] = bitcoin_df[wallet_column].fillna('No Wallet Address')
        bitcoin_df[wallet_column] = bitcoin_df[wallet_column].replace('', 'No Wallet Address')
...
def sender_totals(dataframes):
    if "Attempted P2P Payments" in dataframes:
        ...
        if 'Total' in sent_payments_df.columns and 'Sender' in sent_payments_df.columns:
            ...
            sent_payments_df['Currency'] = sent_payments_df['Total'].str.extract(r'([A-Z]{3})')[0]
            sent_payments_df['Total'] = sent_payments_df['Total'].str.replace(
                r'[A-Z]{3} ', '', regex=True
            ).str.replace(',', '', regex=False).astype(float)

            # Group by 'Sender' and 'Currency' and calculate aggregates
            send_group = sent_payments_df.groupby(['Sender', 'Currency'])
            ...
            for col in [
                ...
                send_stats[col] = send_stats.apply(
                    lambda row: f"{row['Currency']} {row[col]:,.2f}", axis=1
                )
    ...
    "The Excel spreadsheet found by the tool does not appear to contain financial transactions. "
    "Please review the structure of the data you provided. The expected folder structure is:\n\n"
    "Input_Folder/\n"
    "├\n"
    "├── MainData.xlsx  <-- The primary Excel file at root.\n"
    "├\n"
    "├── sender_receiver_accounts Subfolder\n"
    "├   ├── Account_A.xlsx\n"
    "├   └── Account_B.xlsx\n"
    "├\n"
    "├── ID_Photos Subfolder\n"
...
def plot_top_sender_totals(df):
    df_plot = df.copy()
    df_plot['Total_Amount'] = df_plot['Total_Amount'].replace(r'[^\d.]', '', regex=True).astype(float)
    
    usd_senders = df_plot[df_plot['Currency'] == 'USD'].head(10)
    gbp_senders = df_plot[df_plot['Currency'] == 'GBP'].head(10)
    
    # Create chart based on availability of data
    if not usd_senders.empty and not gbp_senders.empty:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(28, 8))
        
        ax1.barh(usd_senders['Sender'], usd_senders['Total_Amount'], color='purple')
        ax1.set_xlabel('Total Amount Sent (USD)')
        ax1.set_ylabel('Sender')
        ax1.set_title('Top 10 Senders by Total Amount (USD)')
        ax1.invert_yaxis()
        
        ax2.barh(gbp_senders['Sender'], gbp_senders['Total_Amount'], color='green')
        ax2.set_xlabel('Total Amount Sent (GBP)')
        ax2.set_ylabel('Sender')
        ax2.set_title('Top 10 Senders by Total Amount (GBP)')
        ax2.invert_yaxis()
    elif not usd_senders.empty:  # Only USD data available
        ...
        ax1.set_title('Top 10 Senders by Total Amount (USD)')
        ax1.invert_yaxis()
    elif not gbp_senders.empty:  # Only GBP data available
        fig, ax1 = plt.subplots(1, 1, figsize=(14, 8))
        ax1.barh(gbp_senders['Sender'], gbp_senders['Total_Amount'], color='green')
        ax1.set_xlabel('Total Amount Sent (GBP)')
        ax1.set_ylabel('Sender')
        ax1.set_title('Top 10 Senders by Total Amount (GBP)')
        ax1.invert_yaxis()
    else:
        # No data available; show a message instead.
        fig, ax1 = plt.subplots(1, 1, figsize=(14, 8))
        ax1.text(0.5, 0.5, "No sender data available", ha='center', va='center', fontsize=14)
        ax1.axis('off')
...
def recipient_totals(dataframes):
    if "Attempted P2P Payments" in dataframes:
        ...
        # Debug: Print the columns and first few rows to verify data
        print("sent_payments_df columns:", sent_payments_df.columns)
        print("sent_payments_df head before cleaning:")
        print(sent_payments_df.head())

        # Ensure that 'Total' and 'Recipient' columns exist
        if 'Total' in sent_payments_df.columns and 'Recipient' in sent_payments_df.columns:
            # Do not re-extract currency if it already exists
            # Clean and convert the "Total" column to numerical values
            # (Assuming Total may have been a string with digits only)
            sent_payments_df['Total'] = sent_payments_df['Total'].astype(str)
            print("Before cleaning, 'Total':")
            print(sent_payments_df['Total'].head())

            # Remove currency prefix if present, otherwise it will remain unchanged
            sent_payments_df['Total'] = sent_payments_df['Total'].str.replace(
                r'[A-Z]{3}\s', '', regex=True
            ).str.replace(',', '', regex=False).astype(float)
            print("Cleaned Total values:")
            print(sent_payments_df['Total'].head())

            # Group by 'Recipient' and use the existing 'Currency' column
            send_group = sent_payments_df.groupby(['Recipient', 'Currency'])
            send_stats = send_group['Total'].agg(
                Total_Amount='sum',
                Number_of_Transactions='count',
                Average_Transaction_Amount='mean',
            ...
            print("Grouped send_stats:")
            print(send_stats.head())

            send_stats = send_stats.sort_values(by='Total_Amount', ascending=False)
            print("send_stats BEFORE formatting:")
            print(send_stats)
            ...
            for col in [
                ...
                numeric_vals = pd.to_numeric(send_stats[col], errors='coerce').fillna(0)
                send_stats[col] = send_stats['Currency'] + ' ' + numeric_vals.map(lambda x: f"{x:,.2f}")

            print("send_stats AFTER formatting:")
            print(send_stats)
            return send_stats
        else:
        ...
        messagebox.showerror("Error","Dataset 'Attempted P2P Payments' not found.")
        return pd.DataFrame()

def plot_recipient_totals(df):
    df_plot = df.copy()
    # Remove a leading currency string (e.g. "USD ") and any commas before converting to float
    df_plot['Total_Amount'] = (
        df_plot['Total_Amount']
        .replace(r'^[A-Z]+\s', '', regex=True)
        .str.replace(',', '')
        .astype(float)
    )
    usd_recipients = df_plot[df_plot['Currency'] == 'USD'].head(10)
    gbp_recipients = df_plot[df_plot['Currency'] == 'GBP'].head(10)
    
    # Create chart based on availability of data
    if not usd_recipients.empty and not gbp_recipients.empty:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(28, 8))
        ax1.barh(usd_recipients['Recipient'], usd_recipients['Total_Amount'], color='darkorange')
        ax1.set_xlabel('Total Amount Received (USD)')
        ax1.set_ylabel('Recipient')
        ax1.set_title('Top 10 Recipients by Total Amount (USD)')
        ax1.invert_yaxis()
        
        ax2.barh(gbp_recipients['Recipient'], gbp_recipients['Total_Amount'], color='blue')
        ax2.set_xlabel('Total Amount Received (GBP)')
        ax2.set_ylabel('Recipient')
        ax2.set_title('Top 10 Recipients by Total Amount (GBP)')
        ax2.invert_yaxis()
    elif not usd_recipients.empty:  # only USD data available
        fig, ax1 = plt.subplots(1, 1, figsize=(14, 8))
        ax1.barh(usd_recipients['Recipient'], usd_recipients['Total_Amount'], color='darkorange')
        ax1.set_xlabel('Total Amount Received (USD)')
        ax1.set_ylabel('Recipient')
        ax1.set_title('Top 10 Recipients by Total Amount (USD)')
        ax1.invert_yaxis()
    elif not gbp_recipients.empty:  # only GBP data available
        fig, ax1 = plt.subplots(1, 1, figsize=(14, 8))
        ax1.barh(gbp_recipients['Recipient'], gbp_recipients['Total_Amount'], color='blue')
        ax1.set_xlabel('Total Amount Received (GBP)')
        ax1.set_ylabel('Recipient')
        ax1.set_title('Top 10 Recipients by Total Amount (GBP)')
        ax1.invert_yaxis()
    else:
        # No data available for either currency
        fig, ax1 = plt.subplots(1, 1, figsize=(14, 8))
...
def create_html_report(
    ...
    # Load the template
    try:
        template = env.get_template('report_template_1_5.html')
    except TemplateNotFound:
        raise FileNotFoundError(f"Template file not found: {os.path.join(template_dir, 'report_template_1_5.html')}")
...
def process_zip_data():
    progress = ProgressHandler(progress_spinner, status_label, root)
    
    try:
        progress.start("Processing ZIP file...")
        zip_path = zip_file_entry.get()
        zip_password = zip_password_entry.get()
        output_folder = output_folder_entry_zip.get()
        progress.update("Extracting ZIP file...")
        ...
        try:
            ...
            return
        progress.update("Extracted ZIP file.")
        current_year = datetime.datetime.now().year
        ...
        progress.update("Report generated.")
    
        messagebox.showinfo("Processing Complete", f"Report generated: {output_path}")
    
    except Exception as e:
        # Log detailed error info to a file in the output directory.
        output_dir = output_folder_entry_folder.get() or os.getcwd()
        log_error(e, output_dir)
        # Show a generic error popup window to the user.
        messagebox.showerror("Error", "An error occurred while processing your data. Detailed error information has been logged to 'Cash_Crawler_Error_log.txt' in the output directory. Please submit it to North Loop Consulting for support.")

    finally:
        progress.stop()

def process_folder_data():
    progress = ProgressHandler(progress_spinner_folder, status_label_folder, root)
    
    try:
        progress.start("Processing folder...")
        ...
        if not folder_path or not output_folder:
            ...
            return
        images_dir = os.path.join(output_folder, 'Images')
        progress.update("Processing folder...")
        os.makedirs(images_dir, exist_ok=True)
        file_path = find_excel_file(folder_path)
        progress.update("Processing Excel data...")        
        if not file_path:
            progress_spinner_folder.stop()
            messagebox.showerror(
                "File Not Found",
        ...
        progress.update("Reading Excel data...")
        dataframes = read_excel_datasets(file_path)
        progress.update("Collecting account data...")
        account_data = collect_account_data(dataframes)
        progress.update("Joining dataframes...")
        joined_df = join_dataframes(dataframes)
        progress.update("Processing Bitcoin transactions...")
        top_bitcoin_stats = bitcoin_transactions(dataframes)
        progress.update("Processing sender data...")
        sender_totals_df = sender_totals(dataframes)
        progress.update("Processing sender data...")
...
ttk.Label(splashpage, text="v1.8  ©2025 North Loop Consulting, LLC", font=('Helvetica', 14)).grid(row=3, column=0, pady=10)
...
page2.grid_columnconfigure(1, weight=2)
page2.grid_columnconfigure(2, weight=1)
page2.grid_rowconfigure(5, weight=1)
page2.grid_rowconfigure(6, weight=1)
...
progress_spinner = ActivityIndicator(page2)
progress_spinner.grid(row=5, column=0, columnspan=3, pady=20, padx=20)
progress_spinner.grid_remove()  # Hide initially
status_label = ttk.Label(page2, text="")
...
progress_spinner_folder = ActivityIndicator(page3)
progress_spinner_folder.grid(row=5, column=0, columnspan=3, pady=10) 
progress_spinner_folder.grid_remove()
status_label_folder = ttk.Label(page3, text="")
status_label_folder.grid(row=6, column=0, columnspan=3, pady=10)
